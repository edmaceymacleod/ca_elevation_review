...
| `CaElevationKit (macos)` | `ios-app/**` | macos | **yes** (SwiftLint gates) |
...
- **Swift:** `swift build`/`swift test` of `CaElevationKit` **block**; **SwiftLint** **gates** — CI installs SwiftLint and runs `swiftlint lint --strict`, so the tree is kept clean and any violation (warnings promoted to errors) fails the build. Run it locally before pushing with `cd ios-app && swiftlint lint --strict --config .swiftlint.yml`. Separately, the **App target** is generated with XcodeGen and built via `xcodebuild` for the iOS Simulator (signing disabled) so **`actool` compiles `Assets.xcassets`** — this catches invalid `AppIcon`/`AccentColor` and app-layer compile errors the kit-only job misses. Build-only: the LiDAR capture path can't run in the Simulator, so nothing is launched or tested on-device.
